...
class CreateUpdateNewPlayListUseCase  @Inject constructor(private val songPlayListRepository: SongPlayListRepository) {
    suspend operator fun invoke(playListTable: PlayListTable): Flow<ResultState<String>> {
        return songPlayListRepository.createUpdateNewPlayList(playListTable = playListTable)
    }
